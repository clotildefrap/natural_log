    ...
    for layer, hidden_states in all_layers_hidden_states.items():
        ...
        for group, reps in hidden_states['hidden_states'].items():
            ...
            for rep, answer in zip(reps, hidden_states['answers'][group]):
               ...
               # Models like LLaMA, Mistral, and Gemma default to bfloat16 weights. 
               # NumPy has no bfloat16 dtype (unlike float16), so the conversion fails. 
               # Casting to float32 before the conversion is the standard fix.
                all_representations.append(rep.detach().to(torch.float32).cpu().numpy()) # Convert tensor to Numpy with float agreement  
...
def analyze_transformed_hidden_states(transformed_hidden_states):
    ...
    for layer, hidden_states in transformed_hidden_states.items():
        ...
        # all_representations = np.array(all_representations).flatten()
        all_representations = np.array([r[0] for r in all_representations]) # grabs the first PC dimension, returns a n_representations x 1D (PC)
