...
class analyzer:
    def __init__(self,args):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(args.model_name, token=False)
        self.model = AutoModelForCausalLM.from_pretrained(args.model_name, token=False).to(self.device)
    ...
    def save_to_file(self):
        with open(self.save_path, "wb") as f:
            ...
            print(f"DEBUG: Saving to {os.path.abspath(self.save_path)}")
            if os.path.exists(os.path.dirname(self.save_path)):
                print("DEBUG: Directory exists.")
            else:
                print("DEBUG: Directory does NOT exist. Creating...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data", type=str, default="numerics")
    parser.add_argument("--device", type=int, default=0)
    parser.add_argument("--groups", type=int, nargs="+", default=[1, 2, 3, 4, 5])
    parser.add_argument("--context", type=str, default="random")
    parser.add_argument("--upper_bound", type=int, default=100)
    parser.add_argument("--Tdim", type=int, default=2)
    parser.add_argument("--runs", type=int, default=1)
    args = parser.parse_args()

    a = analyzer(args)
    a.states = a.collect_states()
    a.results = a.analyze(transform=args.transform)
    a.save_to_file()
